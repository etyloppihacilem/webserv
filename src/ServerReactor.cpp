...
void ServerReactor::deleteClient(int socket_fd, EventHandler &handler) {
    info.log() << "ServerReactor: Delete connection on socket " << socket_fd << " hastimedout "
               << handler.checkTimeout() << "." << std::endl;
...
void ServerReactor::run() {
    ...
    while (g_signal == false) {
        ...
        try {
            errno = 0;
            if ((event_count = epoll_wait(_epoll_fd, events, MAX_TOTAL_CONNECTION, TIMEOUT_TIME)) == -1) {
                if (errno == EINTR)
                    return;
